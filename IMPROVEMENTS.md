...
Status as of 2026-09-17: sections 1 through 5 are done except where marked
`[open]` below. Section 6 is done except frontend unit tests. Section 7 is
untouched by design. Each fix has a test; see the file named next to it.

---

## 1. Bugs (fix first)

Each of these returned a raw 500 instead of a clean error from `DESIGN.md`.
Also fixed while here: FastAPI's own request validation failures (a bad
body or form field) now come back in the `VALIDATION_ERROR` shape with a
`fields` map, instead of FastAPI's default `{"detail": [...]}` list.

### [done] Passwords over 72 bytes crash registration
...
### [done] Corrupted or disguised files crash extraction
...
### [done] PDF export has no error handling
...
### [done] Skill gap numbers can come from the LLM
...
### [done] Uploads are fully read before the size check
...
- [done] **Rate limiter race.** `check()` runs before the analysis and `record()`
...
- [done] **DB session used across threads.** `_prune_old_reports` runs through
...
- [done] **Naive datetimes.** Models store `datetime.now(UTC)` in a `DateTime`
...
- [done] **JSON columns are nullable** in the migration even though the app always
  writes a list. Make them `NOT NULL` with a `'[]'` server default.
- [done] **Embedding model loads on the first request**, not at startup, because
  ...
  `lifespan` hook.
- [done] **Single-letter seed skills.** `C` and `R` in `skill_seeds.txt` match
...
- [done, cached per JD hash] **Two LLM round trips per analysis.** Skill extraction and report
  ...
  more than once.
- [done] **Warn on long analyses.** Log how long each stage takes (extract, skills,
  score, LLM). Without numbers, there's no way to tell whether a slow request
  was Groq, Gemini fallback, or a cold model load.
- [done] **Silence the google-genai AFC warning** by passing an explicit config that
  disables automatic function calling. It prints on every Gemini call and
  clutters logs.
- [done] **Reuse provider clients.** `Groq(...)` and `genai.Client(...)` are created
...
- [done] **Enforce a minimum `JWT_SECRET` length** (32 bytes) in `config.py`. PyJWT
  already warns about short keys in the test run.
- [done] **Drop `allow_credentials=True` from CORS.** Auth is a bearer header, not a
  cookie, so credentials mode isn't needed and only widens what the CORS
  policy allows.
- [done] **Login timing.** `login` skips bcrypt when the email doesn't exist, so
  response time reveals which emails are registered. Verify against a dummy
  hash in that branch.
- [done, per client IP] **Rate limit auth endpoints.** Register and login have no limit, so
  password guessing is unthrottled.
- [done] **Run the container as a non-root user** and add a `HEALTHCHECK` pointing at
  `/api/health`.
...
- [done] **Handle the LLM_UNAVAILABLE case in place.** Right now a failed analysis
  drops the user back to an empty-looking form. Keep their file and JD in
  state and offer a "Try again" button.
- [done] **History pagination.** The API supports `limit` and `offset`, but the
  History page only ever loads the first 20.
- [done] **404 route.** Unknown URLs render a blank page. Add a catch-all route.
- [done] **Errors on ReportDetail replace the whole page**, including after a failed
  PDF download. Show download and delete errors inline and keep the report
  visible.
- [done] **DOCX type check.** Some browsers report an empty MIME type for `.docx`, so
  `FileDrop` rejects valid files. Fall back to the extension when `type` is
  empty; the server still validates by magic bytes.
- [done] **Split `useAuth` into its own file** to clear the fast-refresh lint
  warning.
- [done] **Replace `window.confirm`** for delete with an in-page confirmation.

---

## 6. Testing and tooling

- [done] **Tests for every bug in section 1**, per `RULES.md`: a bug fix comes with
  the test that would have caught it.
- [done for migrations; pytest still runs on SQLite] **Run the suite against Postgres at least once.** Everything so far ran on
  SQLite; JSONB, timezone handling, and constraint errors behave differently.
- [open] **Frontend tests.** None exist. Start with `AuthContext` (no login flash)
  and `lib/errorMessages.js` (every backend code has a message).
- [done, `tests/test_contracts.py`] **A contract test** that every error code in `DESIGN.md` exists in both
  `core/exceptions.py` and `lib/errorMessages.js`, so they can't drift.
- [done; it did break: Debian trixie renamed the package to `libgdk-pixbuf-2.0-0`] **Build the Docker image locally** before the first Render deploy. It has
  never been built, and WeasyPrint's system libraries are the most likely
  break.
- [done, `.github/workflows/ci.yml`] **CI** is deferred in `PHASES.md`, but a single workflow running `pytest`,
